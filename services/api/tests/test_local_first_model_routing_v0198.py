...
_LOCAL_ENV_NAMES = (
    ...
    "PARALLAX_LOCAL_MODEL_CREDENTIAL_PRIMARY",
    "PARALLAX_LOCAL_MODEL_CREDENTIAL_SECONDARY",
)
...
def test_local_provider_failure_falls_back_to_luna_with_sanitized_evidence(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
):
    _enable_local(monkeypatch)
    seen: list[str] = []
    secret_detail = "raw-local-provider-detail-must-not-leak"
    caplog.set_level(logging.WARNING, logger="parallax_api.intelligence.router")
...
def test_local_validation_failure_falls_back_without_weakening_validator(
    ...
    assert [record.provider_kind for record in result.attempts] == ["ollama", "vercel_ai_gateway"]
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "https://local-model.internal.example/v1",
    ),
)
def test_vercel_production_rejects_every_local_first_endpoint(
    monkeypatch: pytest.MonkeyPatch,
    api_base: str,
):
    _enable_local(monkeypatch, api_base=api_base)
    monkeypatch.setenv("VERCEL_ENV", "production")
    with pytest.raises(ModelRouteConfigurationError, match="unavailable in Vercel production"):
        effective_model_order()
...
@pytest.mark.parametrize(
    ...
    (
        ("openai/local-coder", "ollama", "ollama model namespace"),
        ("ollama_chat/qwen3-coder:30b", "openai_compatible", "openai model namespace"),
    ),
)
def test_provider_and_model_namespace_must_match(
    monkeypatch: pytest.MonkeyPatch,
    model: str,
    provider: str,
    message: str,
    ...
    with pytest.raises(ModelRouteConfigurationError, match=message):
        effective_model_order()
...
@pytest.mark.parametrize(
    "api_base",
    (
        "https://user:password@model.internal.example/v1",
        "https://model.internal.example/v1?token=secret",
        "https://model.internal.example/v1#fragment",
        "https://model.internal.example:invalid/v1",
    ),
)
def test_unsupported_endpoint_authority_components_fail_closed(
    monkeypatch: pytest.MonkeyPatch,
    api_base: str,
):
    _enable_local(monkeypatch, api_base=api_base)
...
def test_reserved_hosted_credential_slot_cannot_be_reused(monkeypatch: pytest.MonkeyPatch):
    _enable_local(monkeypatch, api_key_env="AI_GATEWAY_API_KEY", api_key="must-not-be-used")
    with pytest.raises(ModelRouteConfigurationError, match="reserved credential"):
        effective_model_order()


def test_arbitrary_server_secret_slot_cannot_be_selected(monkeypatch: pytest.MonkeyPatch):
    _enable_local(monkeypatch, api_key_env="DATABASE_URL", api_key="database-secret")
    with pytest.raises(ModelRouteConfigurationError, match="dedicated local-model namespace"):
        effective_model_order()


def test_missing_dedicated_local_secret_fails_before_model_execution(monkeypatch: pytest.MonkeyPatch):
    _enable_local(
        monkeypatch,
        api_base="https://local-model.internal.example/v1",
        api_key_env="PARALLAX_LOCAL_MODEL_CREDENTIAL_PRIMARY",
    )
    with pytest.raises(ModelRouteConfigurationError, match="credential is unavailable"):
        ModelRouter[str]()
...
def test_dedicated_local_secret_is_resolved_but_never_logged(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
):
    _enable_local(
...
def test_process_vercel_oidc_token_never_authorizes_hosted_production(
    monkeypatch: pytest.MonkeyPatch,
):
    _clean_env(monkeypatch)
    monkeypatch.setenv("VERCEL_ENV", "production")
    monkeypatch.setenv("VERCEL_OIDC_TOKEN", "process-token-is-not-runtime-authority")
    fake = FakeDspy()
    monkeypatch.setattr(dspy_programs, "_dspy", lambda: fake)
    ...
    with pytest.raises(
        ...
        dspy_programs.build_lm(MODEL_ORDER[0])
    assert fake.calls == []
