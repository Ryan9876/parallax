...
def _prove_production_snapshot_preflight() -> None:
    original = os.environ.get("VERCEL_ENV")
    os.environ["VERCEL_ENV"] = "production"
    try:
        execution_snapshot_preflight()
    finally:
        if original is None:
            os.environ.pop("VERCEL_ENV", None)
        else:
            os.environ["VERCEL_ENV"] = original
...
def main() -> None:
    ...
    _prove_production_snapshot_preflight()
    ...
    try:
        ...
        print(json.dumps({"gate": "exact-lineage offline execution proof", "run_id": RUN_ID, "lineage_id": LINEAGE_ID, "results": results}, indent=2, sort_keys=True))
    finally:
        engine.dispose()
