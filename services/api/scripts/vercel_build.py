...
PROOF_BRANCH = "control/w4-final-source-delivery-proof"
...
def _create_fresh_plan_run(*, token: str) -> str:
    ...
    if (
        ...
        raise RuntimeError(f"fresh source-delivery proof run was not created at SPECIFY revision 0: {created!r}")
    ...
    if (
        ...
        raise RuntimeError(f"fresh source-delivery proof run did not reach PLAN revision 1: {advanced!r}")
    return run_id


def _successful_delivery_event(*, token: str, run_id: str) -> dict[str, Any]:
    ...
    if not isinstance(events, list):
        raise RuntimeError("fresh source-delivery proof returned no persisted event page")

    delivery_events = [
        event for event in events
        if isinstance(event, dict) and event.get("event_type") == "SOURCE_DELIVERY"
    ]
    failed = [
        event for event in delivery_events
        if event.get("outcome") != "SUCCEEDED" or event.get("failure_code") is not None
    ]
    if failed:
        raise RuntimeError(f"fresh source-delivery proof persisted failed/non-success delivery evidence: {failed!r}")
    succeeded = [event for event in delivery_events if event.get("outcome") == "SUCCEEDED"]
    if len(succeeded) != 1:
        raise RuntimeError(f"fresh source-delivery proof requires exactly one successful SOURCE_DELIVERY event: {delivery_events!r}")

    event = succeeded[0]
    metadata = event.get("metadata")
    if not isinstance(metadata, dict):
        raise RuntimeError("successful SOURCE_DELIVERY event has no metadata")
    required_strings = ("branch_name", "commit_revision", "preview_deployment_id", "preview_status")
    for key in required_strings:
        value = metadata.get(key)
        if not isinstance(value, str) or not value:
            raise RuntimeError(f"successful SOURCE_DELIVERY event is missing {key}")
    ...
    if not isinstance(pull_request_number, int) or isinstance(pull_request_number, bool) or pull_request_number <= 0:
        raise RuntimeError("successful SOURCE_DELIVERY event has invalid pull_request_number")
    if metadata.get("preview_status") != "READY":
        raise RuntimeError(f"successful SOURCE_DELIVERY preview is not READY: {metadata.get('preview_status')!r}")
    lineage = event.get("source_lineage_ref")
    if not isinstance(lineage, str) or not lineage.startswith("src:"):
        raise RuntimeError("successful SOURCE_DELIVERY event is missing accepted source lineage")
    return event


def _assert_final_proof(*, token: str, run_id: str, evidence: dict[str, Any]) -> None:
    ...
    if run.get("state") != "REVIEW" or evidence.get("final_state") != "REVIEW":
        raise RuntimeError(
            f"fresh production source-delivery proof stopped before REVIEW: state={run.get('state')!r}, evidence={evidence!r}"
        )
    attempts = run.get("attempts")
    if not isinstance(attempts, list):
        raise RuntimeError("fresh production source-delivery proof returned no protected attempt history")
    ...
    if missing:
        raise RuntimeError(f"fresh production source-delivery proof is missing passed protected stages: {missing}")
    if run.get("last_failure_code"):
        raise RuntimeError(f"fresh production source-delivery proof retained failure code {run.get('last_failure_code')!r}")
    if evidence.get("credential_unavailable_absent") is not True:
        raise RuntimeError("fresh production source-delivery proof did not prove credential failure absence")

    delivery = _successful_delivery_event(token=token, run_id=run_id)
    metadata = delivery["metadata"]
    assert isinstance(metadata, dict)
    print(
        json.dumps(
            {
                "gate": "Wave 4 final production source-delivery proof",
                "run_id": run_id,
                "final_state": run.get("state"),
                "revision": run.get("revision"),
                "passed_required_stages": list(required),
                "accepted_lineage_ref": delivery.get("source_lineage_ref"),
                "event_sequence": delivery.get("sequence"),
                "credential_unavailable_absent": True,
                "source_delivery": {
                    "branch_name": metadata.get("branch_name"),
                    "commit_revision": metadata.get("commit_revision"),
                    "pull_request_number": metadata.get("pull_request_number"),
                    "preview_deployment_id": metadata.get("preview_deployment_id"),
                    "preview_status": metadata.get("preview_status"),
                    "delivery_action_count": metadata.get("delivery_action_count"),
...
def _run_final_proof() -> None:
    ...
    if not token:
        raise RuntimeError("final source-delivery proof requires existing PARALLAX_ACCESS_TOKEN")

    repository_root = Path(__file__).resolve().parents[3]
    proof_script = repository_root / "scripts" / "w4_release_proof.py"
    run_id = _create_fresh_plan_run(token=token)
    print(f"W4 final source-delivery proof setup: PASS (fresh_project_bound_plan_run={run_id})")

    evidence_path = Path("/tmp/w4-final-source-delivery-proof.json")
    env = os.environ.copy()
    ...
    subprocess.run(
        [
            ...
            str(proof_script),
            "--operation-key",
            f"w4-final-source-delivery-proof-{run_id}",
            "--timeout",
            "600",
            "--evidence",
            str(evidence_path),
    ...
    if not isinstance(evidence, dict):
        raise RuntimeError("Wave 4 source-delivery proof evidence is malformed")
    _assert_final_proof(token=token, run_id=run_id, evidence=evidence)
