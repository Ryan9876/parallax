...
Architecture v3.27 adds one exact worker PLAN-reference rebind after a human-authorized failed-IMPLEMENT PLAN refresh so a recovered worker generation can bind to the newly authoritative protected PLAN without weakening ordinary PLAN-reference immutability. Architecture v3.26 remains the human-authorized PLAN-refresh foundation and v3.25 remains the terminal-worker re-arm foundation; hosted model escalation, candidate-recovery, validator-guided repair, source authority, deployment authority, and REVIEW ceilings remain unchanged. Architecture v3.24 remains the authoritative hosted implementation-model escalation foundation, v3.23 remains the authoritative validator-guided repair foundation, v3.22 remains the authoritative bounded implementation-candidate recovery foundation, and v3.21 remains the authoritative empty-greenfield source-authority foundation. The complete v3.19 architecture is incorporated by reference. Every v3.19 durable contract not explicitly changed below remains authoritative, including source/deployment separation, canonical Project / Work Specification / Engineering Run authority, immutable accepted source lineage, single-writer canonical mutation, durable worker recovery, Project-scoped tool/provider authority, protected evaluation, logical workspace deletion/retention, Preview/REVIEW ceilings, governed release evidence, W8-S2 deferred Vercel Project readiness, W9 benchmark admission, governed skill intake, explicit GitHub installation coverage, exact-one-repository runtime credential scope, quota-independent public source bootstrap, repository-aware protected validation, and bounded dependency PREPARE.
...
## Human-authorized worker PLAN rebind after refreshed PLAN

A durable worker checkpoint normally treats its accepted `plan_ref` as immutable. P2-V0.23.13 intentionally creates a new `TeamPlan.plan_id` when an operator explicitly retries a failed agentic IMPLEMENT run through a fresh protected PLAN, so the next recovered worker generation must be able to bind to that exact new PLAN without making PLAN identity generally mutable.

Architecture v3.27 admits one narrow exception at the first protected `AGENT_DISPATCH` checkpoint after the refresh. A changed worker PLAN reference is accepted only when the Engineering Run is currently at IMPLEMENT, the requested reference is canonical `agentic-plan:<sha256>`, and the latest two durable PLAN attempts are exactly a `RESUMED` control attempt carrying `plan_refresh_authorized=true` and `prior_resume_stage=IMPLEMENT`, immediately followed by a PASSED server-owned agentic PLAN whose `team_plan_id` exactly equals the requested digest. This durable sequence is created only by the P2-V0.23.13 authenticated human-resume path. Browser fields, model output, repository content and provider responses cannot manufacture the exception.

The live agentic dispatch path still must obtain a valid protected worker mutation lease before it can reach that checkpoint. Terminal workers use the v3.25 human re-arm path; stale live leases use the established process-loss classification and `STALLED -> RECOVERING -> REASSIGNED` path. The exceptional PLAN-reference validation therefore does not mint a lease, restart a worker, or create recovery authority. Once the first refreshed checkpoint is persisted, its new PLAN reference becomes the ordinary existing reference and all later checkpoints return to exact equality enforcement. A wrong, stale, malformed or non-latest PLAN reference continues to fail closed.

Worker checkpoint contract failures remain pre-mutation contract failures, not raw route errors. `WorkerCheckpointError` retains worker-recovery typing and is additionally recognized by the protected implementation generation boundary as a bounded value-contract error, allowing the existing `ImplementationContractError -> AUTONOMOUS_IMPLEMENT_FAILED` path to keep Engineering Run terminal state consistent with a worker that was stopped before canonical mutation. This does not reclassify unknown exceptions, post-mutation failures, revision conflicts, source acceptance, provider/deployment failures or REVIEW authority.

Historical worker checkpoints, prior PLAN/IMPLEMENT attempts, source-lineage evidence, worker identity and lease-generation history remain immutable evidence. v3.27 changes no source writer, candidate validator, model escalation order, repository/provider credential scope, deployment authority or REVIEW ceiling.