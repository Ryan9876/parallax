...
try {
  ...
  // A non-streaming DOM/state update must also keep the live edge pinned. This models
  // the amendment/spec refresh path that replaces settled conversation content after
  // the active optical response has already completed.
  ...
  // Deliberate operator scroll-away disables automatic following. A later state update
  // must not yank the operator back to the bottom until a new message re-enables follow.
  await response.evaluate((node) => {
    let thread = node.parentElement;
    while (thread) {
      const style = getComputedStyle(thread);
