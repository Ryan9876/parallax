...
const now = '2026-08-25T21:58:18.903014Z';
...
const workSpecification = {
  ...
  objective: 'Apply one bounded visual change.',
  constraints: ['Preserve protected execution boundaries.'],
  acceptance_criteria: ['Keep the change bounded.', 'Preserve mobile usability.'],
  risks: ['Mobile observability must remain reachable.'],
  open_questions: [],
  confidence: 0.99,
  program_version: 'mobile-observability-scroll-smoke',
  model_id: 'test-model',
  created_at: now,
  updated_at: now,
  approved_at: now,
...
const events = [
  ...
  id: `90909090-9090-4090-8090-${String(event.sequence).padStart(12, '0')}`,
  project_id: PROJECT_ID,
  run_id: RUN_ID,
  event_key: `mobile-observer-${event.sequence}`,
  attempt_id: null,
...
function apiServer() {
  return createServer((request, response) => {
    ...
    if (pathname === `/v1/engineering-runs/${RUN_ID}/events` && request.method === 'GET') {
      ...
      const page = events.filter((event) => event.sequence > after);
...
try {
  ...
  await page.getByLabel('Open Live Build observability').click();
  ...
  await scroll.waitFor({ timeout: 5000 });
  await support.waitFor({ state: 'attached', timeout: 5000 });
  ...
  assert(before.scrollHeight > before.clientHeight + 250,
    `mobile observability scroll: outer content is not meaningfully scrollable (${before.scrollHeight} <= ${before.clientHeight})`);

  await support.evaluate((node) => node.scrollIntoView({ block: 'center', behavior: 'instant' }));
  ...
  assert(after.scrollTop > 120, `mobile observability scroll: outer scroll did not advance (${after.scrollTop}px)`);
  assert(supportBox && supportBox.y < after.bottom && supportBox.y + supportBox.height > after.top,
    'mobile observability scroll: lower Activity evidence content is not reachable in the viewport');
  assert(errors.length === 0, `mobile observability scroll: browser errors: ${errors.join(' | ')}`);

  await page.screenshot({ path: `${evidenceDir}/mobile-observability-scroll.png`, fullPage: true });
