...
TESTS_PATH = Path("services/api/tests/test_w8_delivery_readiness.py")
FOCUSED_TEST_PATH = Path("services/api/tests/test_p2_v02336_vercel_repository_discovery.py")
...
def patch_delivery_readiness() -> None:
...
'''
    pattern = re.compile(
        r"    def _matches\(self, \*, github_repo_id: int\) -> list\[dict\[str, object\]\]:\n.*?(?=    def _assert_no_production_deployment)",
        flags=re.DOTALL,
    )
    CODE_PATH.write_text(
        replace_once(code, pattern, replacement, label="_matches"),
        encoding="utf-8",
    )
...
def patch_existing_test() -> None:
    ...
    replacement = '''def test_readiness_reuses_only_exact_numeric_github_repository_match():
...
'''
    pattern = re.compile(
        r"def test_readiness_reuses_only_exact_numeric_github_repository_match\(\):\n.*?(?=def test_missing_target_is_created_once_verified_and_reused_without_production_mutation)",
        flags=re.DOTALL,
    )
    TESTS_PATH.write_text(
        replace_once(tests, pattern, replacement, label="existing readiness test"),
        encoding="utf-8",
    )


def write_focused_tests() -> None:
    FOCUSED_TEST_PATH.write_text('''from __future__ import annotations
